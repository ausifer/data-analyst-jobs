...
-- 3a. How many postings are in Tennessee? How many are there in either Tennessee or Kentucky?
SELECT COUNT(*)
FROM data_analyst_jobs
...
---- Which state shows the highest average rating? NE shows the highest rating
...
GROUP BY company
ORDER BY avg_rating DESC;
---- 10b. What is that rating? American Express at 4.2
...
AND domain IS NOT NULL -- Disregard any postings where the domain is NULL.
ORDER BY hard_to_fill DESC; -- Order your results so that the domain with the greatest number of hard to fill jobs is at the top.
...
-- Count of jobs in top 4 listed for over 3 weeks:
SELECT COUNT(*)
FROM data_analyst_jobs
WHERE skill LIKE '%SQL%'
AND days_since_posting > 21
